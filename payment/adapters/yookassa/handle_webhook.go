...
func (a *YooKassa) HandleWebhook(ctx context.Context, raw []byte, signatureValid bool) (*WebhookResult, error) {
	...
	return a.handlePayload(ctx, webhook, raw, signatureValid)
}

func (a *YooKassa) handlePayload(ctx context.Context, webhook webhookPayload, raw []byte, signatureValid bool) (*WebhookResult, error) {
	mergedCtx, paymentRequestCancel := a.withContext(ctx)
	defer paymentRequestCancel()
	ctx = mergedCtx
