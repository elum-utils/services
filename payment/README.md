...
Worker автоматически создает курсы к `USDT_TON` для активных `crypto_native` и `crypto_jetton` из `payment_asset`. Для самого `USDT_TON` фиксируется курс `1_000_000` micro-USDT. Общая частота задается один раз через `Options.PriceUpdateInterval` и по умолчанию равна 10 минутам. Worker проверяет курсы сразу при запуске, затем через каждый заданный интервал. Он использует chain и contract address актива для DexScreener, объединяет до 30 контрактов в запрос, выбирает наиболее ликвидную пару, защищен DB lease для нескольких нод, восстанавливается после panic и завершается при graceful shutdown.
