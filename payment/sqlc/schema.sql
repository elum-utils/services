...
CREATE TABLE IF NOT EXISTS payment_attempt (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT payment_attempt_idempotency_uq UNIQUE (provider_code, idempotency_key),
...
BEGIN
    IF EXISTS (
        ...
        WHERE conrelid = 'payment_attempt'::regclass
          ...
          AND pg_get_constraintdef(oid) NOT LIKE '%provider_code, idempotency_key%'
    ) THEN
        ALTER TABLE payment_attempt
            DROP CONSTRAINT payment_attempt_idempotency_uq;
        ALTER TABLE payment_attempt
            ADD CONSTRAINT payment_attempt_idempotency_uq UNIQUE (provider_code, idempotency_key);
    END IF;
END
$$;
