...
| `Admin.UpsertComplexCondition(ctx, params)` | `SaveComplexConditionParams{WorkspaceID, ParentTaskID, ConditionTaskID, RequiredStatus, Position, IsRequired}`. | Добавляет или обновляет условие complex task. `RequiredStatus`: `ready` или `claimed`, по умолчанию `ready`. |
