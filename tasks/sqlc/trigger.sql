DROP TRIGGER IF EXISTS task_progress_create_stats;
...
SELECT
    ...
    NEW.platform_user_id,
    NEW.status,
    IF(NEW.status = 'claimed', definition.claim_mode, NULL),
    0,
...
DROP TRIGGER IF EXISTS task_progress_update_stats;
...
SELECT
    ...
    progress.platform_user_id,
    progress.status,
    IF(progress.status = 'claimed', definition.claim_mode, NULL),
    0,
